...
- (id)initWithFrame:(CGRect)frame valueDictionary:(NSDictionary *)valueDictionary
{
    ...
    if (self) {
        ...
        _valueDivider = 1;
#pragma mark - 五芒星颜色
        _drawboardColor = [UIColor whiteColor];
//        _plotColor = [UIColor colorWithRed:.5 green:.5 blue:.5 alpha:.9];
//        _plotColor = [UIColor orangeColor];
        _plotColor = [UIColor colorWithRed:0 green:138.0/255.0 blue:142.0/255.0 alpha:1.0];
...
//    for (NSArray *pointArray in _pointsLengthArrayArray) {
    //得到最外层circle
    NSArray *pointArray = [_pointsLengthArrayArray lastObject];
        //plot background
...
#pragma mark -  将 五芒星 头部文字改为图片
        UIImageView  *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(x - 3.5*fontSize/1.5 + 3, y - fontSize - 10, 4*fontSize, 4*fontSize)];
        imageView.image = [UIImage imageNamed:labelString];
